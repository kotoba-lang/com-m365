(ns m365.main
  "Kotodama WASM entrypoint for the M365 clean-room actor (L4) — Clojure port.
  ...
  facts (`m365.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"m365.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "m365-compat" :tier tier :entities entities} 200])
